...
var Home = function(){
  ...
  const deleteComment= function(postId,commentId){
    ...
    .then(result=>{
        ...
        const newData=data.map(item=>{if(item._id==result._id){
            return result
        }else{
            return item
        }
       ...
       <h6>{item.likes.length} likes</h6>
                ...
                <p>{item.body}</p>

                  {
                       ...
                       return <h6 key={record._id}><span style={{fontWeight:"500"}}>{record.postedBy.name} </span> {record.text} {item.postedBy._id ==state._id 
                        && <i class="material-icons" style={{float:"right"}} 
